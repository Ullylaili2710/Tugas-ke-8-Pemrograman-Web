...
<head>
	...
	<style> 
	.warning {color: #FF0000;} /*membuat class warning yang akan warna merah */
	</style>
</head>
<body>
<?php 
...
if ($_SERVER["REQUEST_METHOD"] == "POST")  {//jika server request method == POST maka
	if (empty($_POST["nama"])) //jika POST nama kosong akan menampilkan pesan error ,nama tidak boleh kosong
	{
	...
	else // jika tidak
	{
		...
		if (!preg_match("/^[a-zA-Z]*$/", $nama)) //jika penulisan input nama tidak sesuai maka akan menampilkan pesan error 
		{
			$nameErr = "Inputan Hanya boleh huruf dan spasi";
		}
	}
	if (empty($_POST["email"])) //jika POST email kosong maka akan menampilkan pesan error email tidak boleh kosong
	{
	...
	else //jika tidak
	{
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)) //jika penulisan email tidak cocok maka akan menampilkan pesan error 
		{
			$error_email = "Format email Invalid";
		}
	}
	if (empty($_POST["pesan"])) //jika POST pesan kosong maka akan menampilkan pesan error pesan tidak boleh kosong
	{
	...
	if (empty($_POST["web"])) //jika POST web kosong maka akan menampilkan pesan error pesan tidak boleh kosong
	{
	...
	else //jika tidak
	{
		...
		if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $web)) // jika penulisan web tidak cocok maka akan menampilkan pesan error
		{
			$error_web = "URL tidak valid";
		}
	}
	if (empty($_POST["telp"]))  //jika POST telp kosong maka akan menampilkan pesan error pesan tidak boleh kosong
	{
